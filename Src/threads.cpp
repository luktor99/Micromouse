...
void StartButtonsPollingTask(void const * argument) {
	...
	for(;;) {
		for(uint8_t i=0; i<5; i++) {
			...
			else if(buttons_debounce[i]==1 || buttons_held[i]==hold_cycles) {
...
void StartRangeSensorsTask(void const * argument) {
	...
	GPIO_InitStructure.Pin = SENSOR1_Pin | SENSOR2_Pin | SENSOR3_Pin | SENSOR4_Pin | SENSOR5_Pin | SENSOR6_Pin | SENSOR7_Pin | SENSOR8_Pin | SENSOR10_Pin | SENSOR11_Pin;
		...
		if(event.status == osEventMessage) {
			...
			if(character == '\r') {
				...
				do {
					...
					status = FreeRTOS_CLIProcessCommand((const char* const)buffer_CLI_RX, buffer_CLI_TX, UART3_TX_bufferlenCLI);
				...
				RX_counter = 0;
				memcpy(buffer_CLI_RX_prev, buffer_CLI_RX, UART3_RX_bufferlenCLI);
				...
				if(RX_counter < UART3_TX_bufferlenCLI) {
					...
					if(RX_counter==1 && buffer_CLI_RX[0]=='[' && character=='A') {
						// print the last command
						print("\b");
						print(buffer_CLI_RX_prev);
						// restore previous command
						memcpy(buffer_CLI_RX, buffer_CLI_RX_prev, UART3_RX_bufferlenCLI);
...
void StartOLEDTask(void const * argument) {
	...
	if(I2C3->SR2 & I2C_SR2_BUSY) I2C3->CR1 = I2C_CR1_SWRST;
	...
	u8g_FirstPage(&u8g);
	do {
		u8g_DrawBitmap(&u8g, 0, 0, bitmap_splash_cnt, bitmap_splash_h, bitmap_splash);
...
void StartLiPoMonitorTask(void const * argument) {
	...
	for(;;) {
		...
		if(adc_lipo[1] > 80 && (adc_lipo[1] < lipo_cell_thresh || adc_lipo[2] < lipo_cell_thresh) ||
				adc_lipo[1] < 80 && (adc_lipo[0] > 525 && adc_lipo[0] < lipo_pack_thresh)) {
...
void m_author1(Menu *m, uint8_t parent) {
	...
	if(show_email) m->current_item->text="luktor99@gmail.com";
	else m->current_item->text="Lukasz Kilaszewski";
...
void m_author2(Menu *m, uint8_t parent) {
	static uint8_t show_email=1;
	if(show_email) m->current_item->text="karol@blueray.com";
	else m->current_item->text="Karol Skorulski";
...
void m_author3(Menu *m, uint8_t parent) {
	static uint8_t show_email=1;
	if(show_email) m->current_item->text="jerzy@blueray.com";
	else m->current_item->text="Jerzy Baranowski";
...
void m_author4(Menu *m, uint8_t parent) {
	static uint8_t show_email=1;
	if(show_email) m->current_item->text="andrzej@blueray.com";
	else m->current_item->text="Andrzej Halicki";
